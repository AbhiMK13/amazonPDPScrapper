...
def Amazon_PDP_Non_Mandatory(batch_name,batch_name_or_df, ip_address):
    # Fo appending the Error IDS to the list
    ERROR_IDS = []
    mandatory =[]
    all_data_list =[]
    ...
    if type(batch_name_or_df)==str:
        ...
        # INPUT_BATCH_FILE = r"C:\Users\User\Desktop\PDP Scraper Exp\batches\{}.xlsx".format(SCRAPED_DATAFILE)
        # INPUT_BATCH_FILE = r"D:\Abhi\Web Scraping\All PdP Files\amazonPDP_env\amazonPDP\amazonPDP\spiders\excep\{}.xlsx".format(SCRAPED_DATAFILE)
        Input_path = os.path.join(BASE_DIR + "\\excep")
        INPUT_BATCH_FILE = r'{0}\\{1}.xlsx'.format(Input_path, SCRAPED_DATAFILE)
        df = pd.read_excel(INPUT_BATCH_FILE)
        # ProductID_List = list(df[0])
        ProductID_List = list(df["Item Code"])
    else:
    ...
    # Comment these lines when you are running ERROR FILE
    ...
    class AmazonPdpSpider(sp.Spider):
        ...
        def start_requests(self):
            for asin in ProductID_List:
                # Formating the producturl with asin numbers
                ...
                yield sp.Request(url=ProductURL, callback=self.parse,   meta={"ASIN": asin,
                                                                        ...
                                                                        headers={"USER_AGENT": user_agent_list[random.randint(0, len(user_agent_list)-1)]})
        ...
        def parse(self, response):
            ...
            try:
                ...
                ERROR_IDS.append(asin)
            print("=========",Output_Data)
            if (Output_Data['Title'] == 'na' and
             Output_Data['Selling Price']=='na' and Output_Data['MRP']=='na'
             and Output_Data['Count of Ratings']=='na' and Output_Data['In Stock'] == 'na'
             and Output_Data['Discount %'] == 'na'):
                ERROR_IDS.append(asin)
                print("\n\npppppppppppppp",asin)
            all_data_list.append(Output_Data)
            # return Output_Data
    ...
    process = CrawlerProcess(settings={'LOG_LEVEL': 'DEBUG',
                                ...
                                # "amazonPDP.amazonPDP.smartproxy_auth.ProxyMiddleware": 543,
    ...
    error_df = pd.DataFrame({"Item Code": list(set(ERROR_IDS))})    # Checing if Error ID file have no ids , then we are removing the file or else we are removing the duplicate IDs
    # if (len(ERROR_IDS) > 0):
    #     try:
    ...
    def Postprocessing(df):
        # try:
        #     for each_id in ERROR_IDS:
        #         df = df[df['Product ID']!=each_id]
        # except:
        #     pass
        try:
            try:
                old_df = pd.read_excel(r"{}\\{}_NonMandatoryData.xlsx".format(no_filtered_dir,batch_name))
                new_df = pd.concat([old_df, df])
                new_df.drop_duplicates(subset="Product ID",
                        keep=False, inplace=True)
                new_df = new_df.replace('na','')
                new_df.to_excel(r"{}\\{}_NonMandatoryData.xlsx".format(no_filtered_dir,batch_name), index=False)
                
            except:
                df.replace('na','')
                df.to_excel(r"{}\\{}_NonMandatoryData.xlsx".format(no_filtered_dir,batch_name), index=False)
            
            # Removing the Data of Title , Product Details from Scraped dataframe if it is not scraped
            #Comment Out this lines if you want to run nonmandatory fields
            # print("======================",df)

            # Inserting the required columns into the dataframe
            # df.insert(6, "Brand", "")
            # df.insert(25, "Available Sizes", "")
            ...
            for row in list(df.index):
                ...
                try:
                    if (df['Bestseller Rank'][row]):
                        ...
                        # print("gggggggggggggggggggggggg",str(b[0]).replace(",",""))
                        # ExtractRank = [
                        #     char for char in df['Bestseller Rank'][row] if char.isdigit()]
                        # Resultrank = int("".join(ExtractRank))
                        c = str(b[0]).replace(",","")
                ...
                # Extracting the brand from Title
                # df["Brand"][row] = df["Title"][row].split(' ')[0]
                if(str(df['Title'][row])=='na' and str(df['Selling Price'][row])=='na' and str(df['MRP'][row])=='na'
                    and str(df['Discount %'][row]) == 'na' and str(df['Count of Ratings'][row]) == 'na'):
                        df['In Stock'][row] = 'Out Of Stock'
                if(df['Title'][row] =='' and df['Selling Price'][row] == ''):
                    df['In Stock'][row] = 'Out Of Stock'
    ...
    # Final output with filtered data try : if already file exists , it will append the data to the previous data , if not it will create a file and append the data.
    # try:
    #     last_df = pd.read_excel(r"D:\Abhi\Web Scraping\All PdP Files\amazonPDP_env\amazonPDP\amazonPDP\spiders\output\{}".format(FINALOUTPUT_FILE))
    #     df = pd.concat([last_df, df])
    # except:
    #     pass
    # scraped_df = scraped_df.replace("na","")
    print("########## Non Mandatory Success ##########")
    winsound.Beep(frequency, duration)
    engine = pyttsx3.init()
    engine.setProperty('rate', 120)
    voices = engine.getProperty('voices')
    engine.setProperty('voice', voices[1].id)
    engine.say("Total Number of Error IDs are {}".format(len(error_df)))
    engine.runAndWait()
    print("\n-----------Error IDs==",len(error_df),"--------------\n")
    # scraped_df.drop_duplicates(subset='Product ID', keep='first', inplace=True)
